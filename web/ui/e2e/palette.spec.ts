...
test("Ctrl-K opens the palette and jumps to a page", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Teams & standings/ }).click();
...
test("palette finds a team by name and by code", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Baltimore Ravens/ }).click();
...
test("> switches to command mode", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: /Density: compact/ })).toBeVisible();
