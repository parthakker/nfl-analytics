...
test("a line chart has a table twin so values are readable without hovering", async ({ page }) => {
  ...
  await expect(figure.getByText("penalties / game")).toBeVisible();
  await expect(figure.getByText("avg total points")).toBeVisible();

  await figure.getByRole("button", { name: "Table" }).click();
  await expect(figure.locator("table thead")).toContainText("Season");
  await expect(figure.locator("table thead")).toContainText("penalties / game");
