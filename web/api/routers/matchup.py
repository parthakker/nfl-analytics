...
def _side(con, team: str, season: int, game_id: str, rest, coach, qb) -> dict:
    ...
    """, [team, season, game_id])[0]
    travel = rows_to_dicts(con, """
...
def h2h(team_a: str, team_b: str, season_min: int | None = None,
        season_max: int | None = None, season_type: str | None = None,
    ...
    if season_min is not None:
        where.append("season >= ?"); params.append(season_min)
    if season_max is not None:
        where.append("season <= ?"); params.append(season_max)
    if season_type:
        where.append("season_type = ?"); params.append(season_type)
    if site == "a_home":
...
def matchup(game_id: str) -> dict:
    with read_conn(attach_kalshi=True) as con:
        ...
        teams = {
            "away": _side(con, away, season, game_id, g["away_rest"],
                          g["away_coach"], g["away_qb_name"]),
            "home": _side(con, home, season, game_id, g["home_rest"],
                          g["home_coach"], g["home_qb_name"]),
        }
        ...
        if g["away_coach"] and g["home_coach"]:
            ...
            if rows:
                meetings = rows_to_dicts(con, """
                    ...
                    ORDER BY game_date DESC LIMIT 5
                """, [g["away_coach"], g["home_coach"]])
                coach_h2h = {"away_coach": g["away_coach"],
                             "home_coach": g["home_coach"],
                             **rows[0], "meetings": meetings}

        referee = None
        if g["referee"]:
            career = rows_to_dicts(con, """
        ...
        market = {"spread_line": g["spread_line"], "total_line": g["total_line"],
                  "home_moneyline": g["home_moneyline"],
                  "away_moneyline": g["away_moneyline"],
                  "line_history": [], "kalshi": None}
        try:
            market["line_history"] = rows_to_dicts(con, """
            ...
            if k:
                m = k[0]
                prob_home = (m["prob"] if m["yes_team"] == home
                             else (1 - m["prob"] if m["prob"] is not None else None))
                market["kalshi"] = {"prob_home": prob_home, "ticker": m["ticker"],
                                    "volume": m["volume"]}
        except Exception:
    ...
    return {
        "game": {k: g[k] for k in ("game_id", "season", "week", "season_type",
                                   "game_type", "date", "gametime", "final",
                                   "away_team", "home_team", "away_score",
                                   "home_score", "overtime", "div_game", "surface")},
        "venue": {k: g[k] for k in ("venue_id", "venue_name", "venue_city",
                                    "venue_country", "roof", "neutral_site",
                                    "is_international")},
        "teams": teams, "coach_h2h": coach_h2h, "referee": referee,
        "weather": weather[0] if weather else None,
        "market": market, "injuries": injuries, "series": series,
    }
