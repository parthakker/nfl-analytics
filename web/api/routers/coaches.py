...
def coach(name: str) -> dict:
    with read_conn() as con:
        ...
        for key, label, table, invert in metrics:
            expr = ("go_attempts::double / nullif(go_situations,0)"
                    if key == "go_rate_calc" else key)
            row = con.execute(f"""
                WITH vals AS (
                ...
                SELECT round(100.0 * count(*) FILTER (
                       ...
                       / nullif(count(*), 0))
                FROM vals
            """, [latest, name]).fetchone()
            fp.append({"metric": label,
                       "pct": int(row[0]) if row and row[0] is not None else None})

        rivals = rows_to_dicts(con, """
    ...
    return {"coach": name, "current_team": team,
            "scheme": meta.get(team) if team else None,
            "seasons": seasons, "fingerprint": fp, "rivals": rivals,
