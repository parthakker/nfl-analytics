...
def board(season: int = 2026, week: int | None = None) -> dict:
    with read_conn(attach_kalshi=True) as con:
        ...
        refs = {r["name"]: r for r in rows_to_dicts(con, """
        ...
        """)}
        coaches = {(r["home_team"]): r["home_coach"] for r in rows_to_dicts(con,
    ...
    for g in games:
        ...
        for k, m in kalshi.items():
            if k[0] == g["away_team"] and k[1] == g["home_team"]:
                ...
                km = {"prob_home": m["prob"] if is_home_market
                      else (1 - m["prob"] if m["prob"] is not None else None),
                      "ticker": m["ticker"], "volume": m["volume"],
                      "spread_width": (round(m["yes_ask"] - m["yes_bid"], 3)
                                       if m["yes_ask"] is not None and m["yes_bid"] is not None else None)}
                break
        ...
        if km and km["prob_home"] is not None and vegas_home is not None:
            ...
            dislocation = {"gap": round(gap, 4), "fee": round(fee, 4),
        ...
        out.append({
            **g,
            ...
            "kalshi": km, "dislocation": dislocation,
            "angles": {
                "rest_diff": (g["home_rest"] - g["away_rest"])
                             if g["home_rest"] is not None and g["away_rest"] is not None else None,
                "div_game": bool(g["div_game"]),
                "referee": ({"name": g["referee"], **{k: ref[k] for k in ("over_rate", "pen_per_game")}}
                            if ref else None),
                "home_coach_ats": ({"coach": hc,
                                    "pct": round(coach_ats[hc]["w"] / coach_ats[hc]["g"], 3)}
                                   if hc and coach_ats.get(hc, {}).get("g") else None),
                "roof": g["roof"],
            },
        })
