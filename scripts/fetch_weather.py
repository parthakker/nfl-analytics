...
def fetch_game(client: httpx.Client, url: str, g: dict, source: str) -> dict | None:
    ...
    try:
        ...
        return None
    idx = min(range(len(times)), key=lambda i: abs(datetime.fromisoformat(times[i]) - ko))
...
def game_pool(backfill: bool) -> list[dict]:
    ...
    try:
        ...
        if backfill:
            ...
            parsed = (
                "LEFT JOIN game_weather_parsed w ON w.game_id = s.game_id"
                if "game_weather_parsed" in have
                else "LEFT JOIN (SELECT NULL AS game_id, NULL AS temp_f) w ON w.game_id = s.game_id"
            )
            sql = f"""
        ...
        return [dict(zip(cols, r, strict=False)) for r in con.execute(sql).fetchall()]
    finally:
        con.close()
...
def main() -> int:
    ...
    save(rows)
    print(
        f"fetch_weather: {source} {got} fetched, {misses} misses, "
