...
DICTIONARY_MAP = {
    "play_by_play": "play_by_play.md", "games": "play_by_play.md",
    "schedules": "schedules.md",
    "players": "player_dimensions.md", "rosters_weekly": "player_dimensions.md",
    "draft_picks": "player_dimensions.md",
    "team_stats": "context_tables.md", "injuries": "context_tables.md",
    "officials": "context_tables.md",
}
for _t in ("player_stats_week", "player_stats_season", "player_stats_def_week",
           "player_stats_def_season", "player_stats_kicking_week",
           "player_stats_kicking_season", "player_stats_week_v2",
           "player_stats_season_v2", "v_player_stats_week_all"):
    DICTIONARY_MAP[_t] = "player_stats.md"
for _t in ("ngs_passing", "ngs_receiving", "ngs_rushing"):
    DICTIONARY_MAP[_t] = "next_gen_stats.md"
for _t in ("advstats_season_pass", "advstats_season_rush", "advstats_season_rec",
           "advstats_season_def", "advstats_week_pass", "advstats_week_rush",
...
def query_warehouse(sql: str, max_rows: int = 200) -> dict:
    ...
    with read_conn(attach_kalshi=True) as con:
        try:
            return table_result(con, stripped, max_rows=max_rows,
                                filters_applied="user SQL, as written")
...
def describe_warehouse(table: str = "") -> dict:
    ...
    with read_conn() as con:
        if not table:
            ...
            """, max_rows=100)
            t["note"] = ("Pass a table name for columns + dictionary notes. "
                         "Global gotchas live in CLAUDE.md; per-table detail "
                         "in docs/dictionary/.")
            return t
        cols = table_result(con, """
...
def player_lookup(name_or_id: str, season: int = 0) -> dict:
    ...
    with read_conn() as con:
        ...
        """, [gsis], filters_applied=f"gsis_id={gsis} {season_filter}")
...
def predict_game(home_team: str, away_team: str) -> dict:
        ...
        if not sched.empty:
            ...
            def _clean(v):
                ...
                if hasattr(v, "item"):   # numpy scalar -> python native
                    return v.item()
                return v
            market = {k: _clean(s[k]) for k in ("game_id", "gameday",
    ...
    out["caveat"] = ("weeks 1-3 ratings lean on prior-season shrinkage; "
...
def power_ratings(top: int = 32) -> dict:
    ...
    with read_conn() as con:
        ...
        """, [top], filters_applied="ratings entering next game, all data through last refresh")
...
def kalshi_markets(kind: str = "game", team: str = "") -> dict:
    ...
    if team:
        t = team.upper()
        ms = [m for m in ms
              if t in (parse_event_ticker(m["event_ticker"]).get("away_team"),
                       parse_event_ticker(m["event_ticker"]).get("home_team"))
              or t in m["ticker"]]
    cols = ["ticker", "title", "yes_team", "yes_bid", "yes_ask",
            "implied_prob_mid", "fee_at_mid", "volume", "open_interest"]
    return {"columns": cols, "rows": [[m[c] for c in cols] for m in ms[:100]],
            "row_count": min(len(ms), 100), "truncated": len(ms) > 100,
            "filters_applied": f"kind={kind}, status=open"
...
def kalshi_price_history(ticker: str) -> dict:
    ...
    with read_conn(attach_kalshi=True) as con:
        try:
            ...
            return {"error": f"no snapshot store yet: {e} — "
...
def player_news(name_or_id: str, days: int = 7) -> dict:
    ...
    with read_conn(attach_news=True) as con:
        ...
        try:
            ...
            """, [gsis, days], filters_applied=f"player={name}, last {days} days")
        except Exception as e:
            ...
            if team:
                return table_result(con, """
                    ...
                    WHERE source = 'espn_news'
                      ...
                      AND published_ts > now() - (? * INTERVAL 1 DAY)
                    ORDER BY published_ts DESC LIMIT 25
                """, [team, days], filters_applied=f"team={team}, last {days} days")
            return table_result(con, """
                SELECT published_ts, headline, body, url FROM newsdb.news
                WHERE source = 'espn_news'
                  AND published_ts > now() - (? * INTERVAL 1 DAY)
                ORDER BY published_ts DESC LIMIT 25
            """, [days], filters_applied=f"all teams, last {days} days")
        except Exception as e:
            return {"error": f"news store unavailable: {e} — run scripts/poll_news.py"}
...
def data_status() -> dict:
    ...
    return {"coverage": cov,
            "kalshi_snapshots": {"count": snaps[0],
