...
FEATURE_COLS = ["d_off_pass", "d_off_rush", "d_def_pass", "d_def_rush",
                "d_rest", "d_tz", "div_game"]
...
def build_features(con: duckdb.DuckDBPyConnection,
                   per_game_ratings: pd.DataFrame) -> pd.DataFrame:
    ...
    df = (games
          .merge(home[["game_id", "team", "h_r_off_pass", "h_r_off_rush",
                       "h_r_def_pass", "h_r_def_rush"]],
                 left_on=["game_id", "home_team"], right_on=["game_id", "team"])
          .drop(columns="team")
          .merge(away[["game_id", "team", "a_r_off_pass", "a_r_off_rush",
                       "a_r_def_pass", "a_r_def_rush"]],
    ...
    df["d_rest"] = (df["home_rest"].fillna(7).clip(3, 14)
                    - df["away_rest"].fillna(7).clip(3, 14))
